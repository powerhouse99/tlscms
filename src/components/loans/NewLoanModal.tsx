...
export function NewLoanModal({ cutoff, onClose, onSuccess }: NewLoanModalProps) {
  ...
  const fetchMembers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter out members with active loans
        const available = data.filter((m: MemberSummaryView) => m.active_loans === 0);
